...
uint8  onenet_server_ip[4] = {163,177,150,12};        //OneNET服务器IP地址
uint16 onenet_server_port  = 1883;                   //OneNET服务器端口号


int MQTT_STATE = MQTT_PKT_CONNECT;   //连接
const char *topics[] = {"test"};
uint8 onenet_ping_pak[2] = {0xC0,0x00};
...
{	
	...
	switch(getSn_SR(SOCK_TCPC))								  				         /*获取socket的状态*/
	{
		...
		case SOCK_INIT:													        	         /*socket处于初始化状态*/
			connect(SOCK_TCPC,onenet_server_ip,onenet_server_port);                /*socket连接服务器*/ 
		break;
	...
	case SOCK_ESTABLISHED: 												             /*socket处于连接建立状态*/
        ...
        OneNet_Subscribe(topics,1);
			...
			case MQTT_PKT_PUBLISH:			
			  ...
			  OneNet_Publish(*topics, "MQTT Publish Test"); //发布消息
			  delay_ms(300);                                 //等待平台响应    
        ...
        MQTT_STATE = MQTT_PKT_PINGREQ;
			...
			case MQTT_PKT_PINGREQ:
				if(onenet_ping_time > 120)
				{
					send(SOCK_TCPC,onenet_ping_pak,2);
					onenet_ping_time = 0;
				}
				break;
		}	
