...
const char AD9834_HELP[] = {
	"AD9834 Task Commands:"
	"\n sys.help(\"ad9834_t\")"
...
u8 AD9834Cmd(AD9834_Dev_T *pDev, const char* CMD, void (*print)(const char* FORMAT_ORG, ...)){
	...
	if(strncmp(CMD, pRsrc->name, nameLen)==0){
		...
		//dev.sine(mHz), in 10E-3 Hz
		else if(sscanf(LINE, ".sine %d ", &num[0])== 1){
			pDev->SineWave(pRsrc, num[0]);
			print("+ok@%s.sine(%d)\r\n", pRsrc->name, num[0]);
			return 1;
		}	
