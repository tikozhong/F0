...
void responseOk_0(u8* rtnStr, u8 rtnStrSz, const u8* FunName);
void responseOk_H(u8* rtnStr, u8 rtnStrSz, const u8* FunName, u32* x);
void responseOk_D(u8* rtnStr, u8 rtnStrSz, const u8* FunName, u32* x);
void responseOk_HH(u8* rtnStr, u8 rtnStrSz, const u8* FunName, u32* x);
void responseOk_DD(u8* rtnStr, u8 rtnStrSz, const u8* FunName, u32* x);



void responseErr(u8* rtnStr, u8 rtnStrSz, const u8* CMD_STR);
...
u8 motorCmd(void* dev, const u8 *cmdStr, u8 *rtnStr, u16 rtnStrSz){
	...
	s8 tmpS8;	u32 val;
	PARAM_DEV_T e; 
	cSPIN_DEV_T *pDev = (cSPIN_DEV_T*)dev;
	cSPIN_Rsrc_T *pRsrc = &pDev->rsrc;
	
	setupParam(&e);
	tmpS8 = fetchFun(cmdStr, &e, NULL, 0);

	//help
	strCpy(funStr, FUN_LEN, pRsrc->name);
	strcat((char*)funStr, (const char*)".help");
	if(isSameStr(e.rsrc.funName, funStr) == 1 ){
		if(tmpS8 == 0){
			strCpy(rtnStr, rtnStrSz, (const u8*)"+ok@");
			strcat((char*)rtnStr, (const char*)pRsrc->name);
			strcat((char*)rtnStr, (const char*)".help() %rtn=\n@{");
			strCatDec(rtnStr, (u32)MOTOR_HELP);
			strcat((char*)rtnStr, (const char*)"@}");	
		}
		else{
			strCpy(rtnStr, rtnStrSz, (const u8*)"+err@");
...
//	//fs.read("filename")
//	strCpy(funStr, FUN_LEN, pRsrc->name);
//	strcat((char*)funStr, (const char*)".read");
//	if(isSameStr(e.rsrc.funName, funStr) == 1 ){
//		strCpy(rtnStr, rtnStrSz, (const u8*)"+ok@");
//		strcat((char*)rtnStr, (const char*)cmdStr);	strReplace(rtnStr, rtnStrSz, endStr, (const u8*)"");
//		strcat((char*)rtnStr, (const char*)" %=");
//		len = strlen((char*)rtnStr);
//		if(pDev->StartRead(&pDev->rsrc,e.rsrc.params[0],&rtnStr[len], rtnStrSz-len) >= 0){
...
//	}
	...
	************************************************************/
	strCpy(funStr, FUN_LEN, pRsrc->name);
	strcat((char*)funStr, (const char*)".set_abs_pos");
	if(isSameStr(e.rsrc.funName, funStr) == 1 ){
		if(pDev->RegSetAbsPos(pRsrc, *(u32*)e.rsrc.params[0]) == CMD_PASS)
			responseOk_0(rtnStr, rtnStrSz, funStr);
		else	responseErr(rtnStr, rtnStrSz, cmdStr);
		return 1;
	}
	/***********************************************************
	@	%name.get_abs_pos()
	...
	************************************************************/
	strCpy(funStr, FUN_LEN, pRsrc->name);
	strcat((char*)funStr, (const char*)".get_abs_pos");
	if(isSameStr(e.rsrc.funName, funStr) == 0 ){
		if(pDev->RegGetAbsPos(pRsrc, &val) == CMD_PASS)
			responseOk_0(rtnStr, rtnStrSz, funStr);
		else	responseErr(rtnStr, rtnStrSz, cmdStr);
		return 1;
	}
...
void responseOk_0(u8* rtnStr, u8 rtnStrSz, const u8* FunName){
	strCpy(rtnStr, rtnStrSz, (const u8*)"+ok@");
	strcat((char*)rtnStr, (const char*)FunName);
	strcat((char*)rtnStr, (const char*)"()");
	strcat((char*)rtnStr, (const char*)endStr);
}
...
void responseOk_DD(u8* rtnStr, u8 rtnStrSz, const u8* FunName, u32* x){
	...
	strcat((char*)rtnStr, (const char*)FunName);
	strcat((char*)rtnStr, (const char*)"(");
	strCatDec(rtnStr, x[0]);	strcat((char*)rtnStr, (const char*)",");
	strCatDec(rtnStr, x[1]);
	strcat((char*)rtnStr, (const char*)")");
	strcat((char*)rtnStr, (const char*)endStr);
}
void responseOk_HH(u8* rtnStr, u8 rtnStrSz, const u8* FunName, u32* x){
	...
	strcat((char*)rtnStr, (const char*)FunName);
	strcat((char*)rtnStr, (const char*)"(");
	strCatHex32(rtnStr, x[0]);	strcat((char*)rtnStr, (const char*)",");
	strCatHex32(rtnStr, x[1]);
	strcat((char*)rtnStr, (const char*)")");
	strcat((char*)rtnStr, (const char*)endStr);
}
...
void responseErr(u8* rtnStr, u8 rtnStrSz, const u8* CMD_STR){
	strCpy(rtnStr, rtnStrSz, (const u8*)"+err@");
	strcat((char*)rtnStr, (const char*)CMD_STR);
}
