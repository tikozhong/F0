...
%name.get_status()
*******************************************************************************/
u8 stepperCmd(cSPIN_DEV_T* pDev, PAKET_T *packetIn, PAKET_T *packetOut){
	u32 i,j,ii,jj;
	cSPIN_Rsrc_T* pRsrc = &pDev->rsrc;
	
	//%devName.help()
	if(isSameStr(packetIn->addr[1], (const u8*)"help") && packetIsMatch(packetIn, "%s%s")){
		packetAddTo(packetOut, "%p", STEPPER_HELP);
		return 1;
	}
	/***********************************************************
	@	_set_param() Group
	...
	************************************************************/
	else if(isSameStr(packetIn->addr[1], (const u8*)"set_abs_pos") && packetIsMatch(packetIn, "%s%s%u")){
		if(pDev->RegSetAbsPos(pRsrc, *(u32*)packetIn->addr[2]) == TRUE)
	...
	@ return	:
	************************************************************/
	else if(isSameStr(packetIn->addr[1], (const u8*)"get_abs_pos") && packetIsMatch(packetIn, "%s%s")){
		if(pDev->RegGetAbsPos(pRsrc, &i) == TRUE)
			RESPONSE(packetOut, OK, (char*)pDev->rsrc.name, "get_abs_pos", "%8d", i);
		else	RESPONSE_ORG(packetOut, ERR, packetIn);
		return 1;
	}
