...
/**
  ...
  * @retval None
  */
void cSPIN_Run(cSPIN_Rsrc_T* pRsrc, cSPIN_Direction_TypeDef direction, u32 speed)
{
...
/**
  ...
  * @retval None
  */
void cSPIN_Move(cSPIN_Rsrc_T* pRsrc, cSPIN_Direction_TypeDef direction, u32 n_step)
{
...
/**
  ...
  * @retval None
  */
void cSPIN_Go_To_Dir(cSPIN_Rsrc_T* pRsrc, cSPIN_Direction_TypeDef direction, u32 abs_pos)
{
...
/**
  ...
  * @retval None
  */
void cSPIN_Go_Until(cSPIN_Rsrc_T* pRsrc, cSPIN_Action_TypeDef action, cSPIN_Direction_TypeDef direction, u32 speed)
{
...
void cSPIN_tickExe(cSPIN_Rsrc_T* pRsrc){
	...
	if(pRsrc->tick > MOTOR_APP_TICK_IN_4MS){
		...
		pRsrc->refLSeries <<= 1;
		pRsrc->refRSeries <<= 1;
		if(HAL_GPIO_ReadPin(pRsrc->nRefL.GPIOx, pRsrc->nRefL.GPIO_Pin) == GPIO_PIN_SET)
			pRsrc->refLSeries |= 1;
		if(HAL_GPIO_ReadPin(pRsrc->nRefR.GPIOx, pRsrc->nRefR.GPIO_Pin) == GPIO_PIN_SET)
			pRsrc->refRSeries |= 1;
