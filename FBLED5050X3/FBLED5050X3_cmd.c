...
/* Private typedef -----------------------------------------------------------*/
/* Private define ------------------------------------------------------------*/
const u8 FBLED5050X3_LED_HELP[] = {
	"\nFBLED5050X3 Task Commands:"
	"\n sys.help(\"FBLED5050X3_t\")"
	"\nFBLED5050X3 Task Commands:"
...
//	FBUSB16X1_Rsrc_T* pRsrc;
	...
	if(isSameStr((const u8*)packetIn->addr[0], (const u8*)pDev->rsrc.name)){
		...
		if(isSameStr(packetIn->addr[1], (const u8*)"help") && packetIsMatch(packetIn, "%s%s")){
			...
			packetAddTo(packetOut, "%s%s%p", str, "help", FBLED5050X3_LED_HELP);
			return 1;
		}
